...
Field `protocol` is `floor.b2a/v1`. Field `settlement` is `not_settled`. Each item has `payment.checkout_url` and/or x402 `payment.accepts`. The house list includes the FLOOR desk (Whop checkout and public x402 receive addresses). Discovery: `GET /.well-known/x402`. Public page for one item: `/l/{sku}` (HTML + embedded item JSON). Human index: `/listings`. If the address is 404, skip. Do not invent sales numbers.
